...
from dpvo.stream import image_stream, image_stream_start_stop
...
@torch.no_grad()
def run(cfg, network, imagedir, calib, stride=1, viz=False, show_img=False, imstart=0, imstop=-1):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default="config/fpv.yaml")#注意这里的config文件
    parser.add_argument('--stride', type=int, default=1)
    parser.add_argument('--skip', type=int, default=0)#注意这里的skip,大概从第700帧开始有运动
    parser.add_argument('--imstart', type=int, default=662)
    parser.add_argument('--imstop', type=int, default=1824)
    ...
    for scene in fpv_scenes:
        ...
        for i in range(args.trials):
            ...
            if args.plot:
                ...
                pdfname = f"trajectory_plots/fpv_{scene}_Trial{i+1:02d}.pdf"
                plot_trajectory(traj_est, traj_ref, f"Euroc {scene} Trial #{i+1} {res_str})",
                                pdfname, align=True, correct_scale=True)

            if args.save_trajectory:
                # Path("saved_trajectories").mkdir(exist_ok=True)
                file_interface.write_tum_trajectory_file(f"saved_trajectories/fpv_{scene}_ATEm_{ate_score:.03f}_MPE_{MPE:.02f}.txt", traj_est)
